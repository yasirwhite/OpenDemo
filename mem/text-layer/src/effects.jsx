...
export function TypeOn({
  ...
  caret = true, exitAt = null, exitDur = 0.434,
}) {
  const chars = [...text];
 ...
 * line. Swap this for an <img>/<svg> at the same cx/cy and the lockup keeps its
 * centring.
 */
export function LogoSlot({
  t, from, to, cx, cy, w = 86, h = 62, label = "logo", ink = C.ink, scale = 1,
}) {
  if (t < from - 0.001 || t > to + 0.001) return null;
  ...
  return (
    <div style={{
      ...
      border: `2px dashed ${rgb(mixRgb([255, 255, 255], ink, 0.35))}`,
      borderRadius: h * 0.22,
      display: "flex", alignItems: "center", justifyContent: "center",
      fontFamily: FONT, fontSize: h * 0.30, fontWeight: 450,
      color: rgb(mixRgb([255, 255, 255], ink, 0.6)), letterSpacing: "0.04em",
    }}>
      {label}
    </div>
  );
