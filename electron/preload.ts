...
// Headless export mode: set when run-demo.mjs spawns OpenScreen for auto-export.
const HEADLESS_EXPORT_PROJECT_PREFIX = "--headless-export-project=";
const HEADLESS_EXPORT_OUT_PREFIX = "--headless-export-out=";
const headlessExportProjectPath =
	process.argv.find((a) => a.startsWith(HEADLESS_EXPORT_PROJECT_PREFIX))
		?.slice(HEADLESS_EXPORT_PROJECT_PREFIX.length) ?? null;
const headlessExportOut =
	process.argv.find((a) => a.startsWith(HEADLESS_EXPORT_OUT_PREFIX))
		?.slice(HEADLESS_EXPORT_OUT_PREFIX.length) ?? null;

contextBridge.exposeInMainWorld("electronAPI", {
	assetBaseUrl,
	// Headless export mode (non-null means auto-export and quit on load)
	headlessExportProjectPath,
	headlessExportOut,
	reportHeadlessExportDone: (result: { outputPath?: string; error?: string }) => {
		ipcRenderer.send("headless-export-done", result);
	},
