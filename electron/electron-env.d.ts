...
interface Window {
	electronAPI: {
		...
		assetBaseUrl: string;
		// Headless export mode — non-null when spawned by run-demo.mjs
		headlessExportProjectPath: string | null;
		headlessExportOut: string | null;
		reportHeadlessExportDone: (result: { outputPath?: string; error?: string }) => void;
		storeRecordedVideo: (
