...
const HEADLESS_EXPORT_PROJECT = process.env["HEADLESS_EXPORT_PROJECT"] ?? "";
const HEADLESS_EXPORT_OUT = process.env["HEADLESS_EXPORT_OUT"] ?? "";
...
export function createEditorWindow(): BrowserWindow {
	...
	const win = new BrowserWindow({
		...
		webPreferences: {
			...
			additionalArguments: [
				ASSET_BASE_URL_ARG,
				...(HEADLESS_EXPORT_PROJECT ? [`--headless-export-project=${HEADLESS_EXPORT_PROJECT}`] : []),
				...(HEADLESS_EXPORT_OUT ? [`--headless-export-out=${HEADLESS_EXPORT_OUT}`] : []),
			],
			nodeIntegration: false,
			contextIsolation: true,
			webSecurity: false,
	...
	// maximize() on Windows forces a hidden window visible — skip in headless mode
